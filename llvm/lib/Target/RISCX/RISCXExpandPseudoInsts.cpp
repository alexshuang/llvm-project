//===-- RISCXExpandPseudoInsts.cpp - Expand pseudo instructions -----------===//
//
...
#define RISCX_EXPAND_PSEUDO_NAME "RISCX pseudo instruction expansion pass"

namespace {

class RISCXExpandPseudo : public MachineFunctionPass {
public:
  const RISCXInstrInfo *TII;
  static char ID;

  RISCXExpandPseudo() : MachineFunctionPass(ID) {
    initializeRISCXExpandPseudoPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return RISCX_EXPAND_PSEUDO_NAME; }
...
char RISCXExpandPseudo::ID = 0;

bool RISCXExpandPseudo::runOnMachineFunction(MachineFunction &MF) {
  TII = static_cast<const RISCXInstrInfo *>(MF.getSubtarget().getInstrInfo());
  bool Modified = false;
  for (auto &MBB : MF)
...
bool RISCXExpandPseudo::expandMBB(MachineBasicBlock &MBB) {
  bool Modified = false;
...
bool RISCXExpandPseudo::expandMI(MachineBasicBlock &MBB,
                                 MachineBasicBlock::iterator MBBI,
                                 MachineBasicBlock::iterator &NextMBBI) {
...
static void doAtomicBinOpExpansion(const RISCXInstrInfo *TII, MachineInstr &MI,
  ...
  case AtomicRMWInst::Nand:
    BuildMI(LoopMBB, DL, TII->get(RISCX::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    BuildMI(LoopMBB, DL, TII->get(RISCX::XORI), ScratchReg)
        .addReg(ScratchReg)
        .addImm(-1);
    break;
  }
...
static void insertMaskedMerge(const RISCXInstrInfo *TII, DebugLoc DL,
  ...
  BuildMI(MBB, DL, TII->get(RISCX::XOR), ScratchReg)
      .addReg(OldValReg)
      .addReg(NewValReg);
  BuildMI(MBB, DL, TII->get(RISCX::AND), ScratchReg)
      .addReg(ScratchReg)
      .addReg(MaskReg);
  BuildMI(MBB, DL, TII->get(RISCX::XOR), DestReg)
      .addReg(OldValReg)
      .addReg(ScratchReg);
}

static void doMaskedAtomicBinOpExpansion(
    const RISCXInstrInfo *TII, MachineInstr &MI, DebugLoc DL,
  ...
  case AtomicRMWInst::Xchg:
    BuildMI(LoopMBB, DL, TII->get(RISCX::ADDI), ScratchReg)
        .addReg(IncrReg)
        .addImm(0);
    break;
  case AtomicRMWInst::Add:
    BuildMI(LoopMBB, DL, TII->get(RISCX::ADD), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Sub:
    BuildMI(LoopMBB, DL, TII->get(RISCX::SUB), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Nand:
...
bool RISCXExpandPseudo::expandAtomicBinOp(
...
static void insertSext(const RISCXInstrInfo *TII, DebugLoc DL,
                       MachineBasicBlock *MBB, Register ValReg,
                       Register ShamtReg) {
  BuildMI(MBB, DL, TII->get(RISCX::SLL), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
  BuildMI(MBB, DL, TII->get(RISCX::SRA), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
}

bool RISCXExpandPseudo::expandAtomicMinMaxOp(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    AtomicRMWInst::BinOp BinOp, bool IsMasked, int Width,
    MachineBasicBlock::iterator &NextMBBI) {
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW32(Ordering)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(RISCX::AND), Scratch2Reg)
      .addReg(DestReg)
      .addReg(MaskReg);
  BuildMI(LoopHeadMBB, DL, TII->get(RISCX::ADDI), Scratch1Reg)
      .addReg(DestReg)
      .addImm(0);
  ...
  case AtomicRMWInst::Max: {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(RISCX::BGE))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  case AtomicRMWInst::UMax:
    BuildMI(LoopHeadMBB, DL, TII->get(RISCX::BGEU))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  BuildMI(LoopTailMBB, DL, TII->get(getSCForRMW32(Ordering)), Scratch1Reg)
      .addReg(AddrReg)
      .addReg(Scratch1Reg);
  BuildMI(LoopTailMBB, DL, TII->get(RISCX::BNE))
      .addReg(Scratch1Reg)
      .addReg(RISCX::X0)
      .addMBB(LoopHeadMBB);
...
bool RISCXExpandPseudo::expandAtomicCmpXchg(
  ...
  if (!IsMasked) {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(RISCX::BNE))
        .addReg(DestReg)
        .addReg(CmpValReg)
        .addMBB(DoneMBB);
    // .looptail:
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(RISCX::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(MaskReg);
    BuildMI(LoopHeadMBB, DL, TII->get(RISCX::BNE))
        .addReg(ScratchReg)
...
bool RISCXExpandPseudo::expandAuipcInstPair(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
  ...
  BuildMI(NewMBB, DL, TII->get(RISCX::AUIPC), DestReg)
      .addDisp(Symbol, 0, FlagsHi);
  BuildMI(NewMBB, DL, TII->get(SecondOpcode), DestReg)
      .addReg(DestReg)
      .addMBB(NewMBB, RISCXII::MO_PCREL_LO);
...
bool RISCXExpandPseudo::expandLoadLocalAddress(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    MachineBasicBlock::iterator &NextMBBI) {
  return expandAuipcInstPair(MBB, MBBI, NextMBBI, RISCXII::MO_PCREL_HI,
                             RISCX::ADDI);
}

bool RISCXExpandPseudo::expandLoadAddress(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    MachineBasicBlock::iterator &NextMBBI) {
  MachineFunction *MF = MBB.getParent();
  ...
  if (MF->getTarget().isPositionIndependent()) {
    ...
    FlagsHi = RISCXII::MO_PCREL_HI;
  }
  return expandAuipcInstPair(MBB, MBBI, NextMBBI, FlagsHi, SecondOpcode);
}

bool RISCXExpandPseudo::expandLoadTLSIEAddress(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    MachineBasicBlock::iterator &NextMBBI) {
  MachineFunction *MF = MBB.getParent();
...
bool RISCXExpandPseudo::expandLoadTLSGDAddress(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    MachineBasicBlock::iterator &NextMBBI) {
  return expandAuipcInstPair(MBB, MBBI, NextMBBI, RISCXII::MO_TLS_GD_HI,
                             RISCX::ADDI);
}

} // end of anonymous namespace

INITIALIZE_PASS(RISCXExpandPseudo, "riscx-expand-pseudo",
                RISCX_EXPAND_PSEUDO_NAME, false, false)
namespace llvm {

FunctionPass *createRISCXExpandPseudoPass() { return new RISCXExpandPseudo(); }
