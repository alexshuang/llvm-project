//===-- RISCXFrameLowering.cpp - RISCX Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the RISCX implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
void RISCXFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  if (isInt<12>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(RISCX::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
  } else {
...
static Register getFPReg(const RISCXSubtarget &STI) { return RISCX::X8; }

// Returns the register used to hold the stack pointer.
static Register getSPReg(const RISCXSubtarget &STI) { return RISCX::X2; }

void RISCXFrameLowering::emitPrologue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<RISCXMachineFunctionInfo>();
  const RISCXRegisterInfo *RI = STI.getRegisterInfo();
  const RISCXInstrInfo *TII = STI.getInstrInfo();
  ...
  Register BPReg = RISCXABI::getBPReg();
  ...
  if (hasFP(MF)) {
    ...
    if (RI->needsStackRealignment(MF)) {
      ...
      if (isInt<12>(-(int)MaxAlignment)) {
        ...
        Register VR =
            MF.getRegInfo().createVirtualRegister(&RISCX::GPRRegClass);
...
int RISCXFrameLowering::getFrameIndexReference(const MachineFunction &MF,
                                               int FI,
                                               unsigned &FrameReg) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo *RI = MF.getSubtarget().getRegisterInfo();
  const auto *RVFI = MF.getInfo<RISCXMachineFunctionInfo>();
...
void RISCXFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  if (hasBP(MF))
    SavedRegs.set(RISCXABI::getBPReg());
  ...
  if (MF.getFunction().hasFnAttribute("interrupt") && MFI.hasCalls()) {
    ...
    static const MCPhysReg CSRegs[] = { RISCX::X1,      /* ra */
      RISCX::X5, RISCX::X6, RISCX::X7,                  /* t0-t2 */
      RISCX::X10, RISCX::X11,                           /* a0-a1, a2-a7 */
      RISCX::X12, RISCX::X13, RISCX::X14, RISCX::X15, RISCX::X16, RISCX::X17,
      RISCX::X28, RISCX::X29, RISCX::X30, RISCX::X31, 0 /* t3-t6 */
    };

    for (unsigned i = 0; CSRegs[i]; ++i)
      SavedRegs.set(CSRegs[i]);

    if (MF.getSubtarget<RISCXSubtarget>().hasStdExtD() ||
        MF.getSubtarget<RISCXSubtarget>().hasStdExtF()) {

      // If interrupt is enabled, this list contains all FP registers.
      const MCPhysReg * Regs = MF.getRegInfo().getCalleeSavedRegs();

      for (unsigned i = 0; Regs[i]; ++i)
        if (RISCX::FPR32RegClass.contains(Regs[i]) ||
            RISCX::FPR64RegClass.contains(Regs[i]))
          SavedRegs.set(Regs[i]);
    }
  }
}

void RISCXFrameLowering::processFunctionBeforeFrameFinalized(
...
MachineBasicBlock::iterator RISCXFrameLowering::eliminateCallFramePseudoInstr(
...
//   add     sp,sp,-64
uint64_t
RISCXFrameLowering::getFirstSPAdjustAmount(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const std::vector<CalleeSavedInfo> &CSI = MFI.getCalleeSavedInfo();
  uint64_t StackSize = MFI.getStackSize();
