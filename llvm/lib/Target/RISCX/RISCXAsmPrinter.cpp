//===-- RISCXAsmPrinter.cpp - RISCX LLVM assembly writer ------------------===//
//
...
// of machine-dependent LLVM code to the RISCX assembly language.
//
//===----------------------------------------------------------------------===//
...
public:
  ...
  bool lowerOperand(const MachineOperand &MO, MCOperand &MCOp) const {
    return LowerRISCXMachineOperandToMCOperand(MO, MCOp, *this);
  }
};
}
...
void RISCXAsmPrinter::EmitInstruction(const MachineInstr *MI) {
  ...
  MCInst TmpInst;
  LowerRISCXMachineInstrToMCInst(MI, TmpInst, *this);
  EmitToStreamer(*OutStreamer, TmpInst);
}

bool RISCXAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
...
bool RISCXAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
  ...
  if (!ExtraCode) {
    ...
    OS << "0(" << RISCXInstPrinter::getRegisterName(MO.getReg()) << ")";
    return false;
  }
