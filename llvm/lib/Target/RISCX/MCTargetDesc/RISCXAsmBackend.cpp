//===-- RISCXAsmBackend.cpp - RISCX Assembler Backend ---------------------===//
//
...
bool RISCXAsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                            const MCFixup &Fixup,
  ...
  return STI.getFeatureBits()[RISCX::FeatureRelax] || ForceRelocs;
}

bool RISCXAsmBackend::fixupNeedsRelaxationAdvanced(const MCFixup &Fixup,
                                                   bool Resolved,
                                                   uint64_t Value,
...
void RISCXAsmBackend::relaxInstruction(const MCInst &Inst,
...
unsigned RISCXAsmBackend::getRelaxedOpcode(unsigned Op) const {
  switch (Op) {
  default:
    return Op;
  case RISCX::C_BEQZ:
...
bool RISCXAsmBackend::mayNeedRelaxation(const MCInst &Inst,
                                        const MCSubtargetInfo &STI) const {
  return getRelaxedOpcode(Inst.getOpcode()) != Inst.getOpcode();
}

bool RISCXAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count) const {
  bool HasStdExtC = STI.getFeatureBits()[RISCX::FeatureStdExtC];
...
bool RISCXAsmBackend::evaluateTargetFixup(
...
bool RISCXAsmBackend::shouldInsertExtraNopBytesForCodeAlign(
...
// We need to insert R_RISCX_ALIGN relocation type to indicate the
// position of Nops and the total bytes of the Nops have been inserted
// when linker relaxation enabled.
// The function insert fixup_riscx_align fixup which eventually will
// transfer to R_RISCX_ALIGN relocation type.
bool RISCXAsmBackend::shouldInsertFixupForCodeAlign(MCAssembler &Asm,
                                                    const MCAsmLayout &Layout,
                                                    MCAlignFragment &AF) {
  ...
  // Create fixup_riscx_align fixup.
  MCFixup Fixup =
      MCFixup::create(0, Dummy, MCFixupKind(RISCX::fixup_riscx_align), SMLoc());
...
RISCXAsmBackend::createObjectTargetWriter() const {
  return createRISCXELFObjectWriter(OSABI, Is64Bit);
}
...
MCAsmBackend *llvm::createRISCXAsmBackend(const Target &T,
  ...
  return new RISCXAsmBackend(STI, OSABI, TT.isArch64Bit(), Options);
}
