//===-- RISCXDisassembler.cpp - Disassembler for RISCX --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the RISCXDisassembler class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "riscx-disassembler"

typedef MCDisassembler::DecodeStatus DecodeStatus;

namespace {
class RISCXDisassembler : public MCDisassembler {

public:
  RISCXDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx)
...
static MCDisassembler *createRISCXDisassembler(const Target &T,
                                               const MCSubtargetInfo &STI,
                                               MCContext &Ctx) {
  return new RISCXDisassembler(STI, Ctx);
}

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeRISCXDisassembler() {
...
static DecodeStatus DecodeGPRRegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  const FeatureBitset &FeatureBits =
      static_cast<const MCDisassembler *>(Decoder)
          ->getSubtargetInfo()
          .getFeatureBits();
  bool IsRV32E = FeatureBits[RISCX::FeatureRV32E];

  if (RegNo >= 32 || (IsRV32E && RegNo >= 16))
    return MCDisassembler::Fail;

  Register Reg = RISCX::X0 + RegNo;
...
static DecodeStatus decodeFRMArg(MCInst &Inst, uint64_t Imm,
  ...
  if (!llvm::RISCXFPRndMode::isValidRoundingMode(Imm))
    return MCDisassembler::Fail;
...
#include "RISCXGenDisassemblerTables.inc"
...
DecodeStatus RISCXDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
