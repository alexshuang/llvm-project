//===----- RISCXMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define DEBUG_TYPE "riscx-merge-base-offset"
#define RISCX_MERGE_BASE_OFFSET_NAME "RISCX Merge Base Offset"
namespace {

struct RISCXMergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  RISCXMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return RISCX_MERGE_BASE_OFFSET_NAME;
  }
...
char RISCXMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(RISCXMergeBaseOffsetOpt, "riscx-merge-base-offset",
                RISCX_MERGE_BASE_OFFSET_NAME, false, false)
...
bool RISCXMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != RISCX::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != RISCXII::MO_HI ||
  ...
  if (LoADDI->getOpcode() != RISCX::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != RISCXII::MO_LO ||
...
// output from LoADDI.
void RISCXMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool RISCXMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               Register GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == RISCX::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == RISCX::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != RISCXII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != RISCX::LUI ||
        LuiImmOp.getTargetFlags() != RISCXII::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == RISCX::LUI) {
...
bool RISCXMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool RISCXMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createRISCXMergeBaseOffsetOptPass() {
  return new RISCXMergeBaseOffsetOpt();
}
